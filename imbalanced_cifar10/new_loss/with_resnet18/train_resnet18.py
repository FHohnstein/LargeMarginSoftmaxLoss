...
#from imbalanced_cifar10.imbalance_cifar import IMBALANCECIFAR10
from imbalance_cifar import IMBALANCECIFAR10

#from imbalanced_cifar10.new_loss.large_margin_softmax import LargeMarginSoftmaxLoss, AdaptedResNet18
from large_margin_softmax import LargeMarginSoftmaxLoss, AdaptedResNet18
...
IMBALANCE_TYPE = "step"
IMBALANCE_FACTOR = 0.33

LargeMarginSoftmaxLoss_M = 2
...
def load_CIFAR10(batch_size: int) -> (DataLoader, DataLoader, DataLoader):
    training_dataset: Dataset = IMBALANCECIFAR10(root='../../../dataset',
                                                 train=True,
                                                 download=True,
...
def fit(model: torch.nn.Module, criterion: Callable, optimizer: torch.optim.Optimizer,
        training_data: DataLoader) -> {str, float}:
    ...
    for samples, labels in training_data:
        ...
        output, features, weights = model(samples)
        loss = criterion(output, features, weights, labels)
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
...
def evaluate(model: torch.nn.Module,  criterion: Callable, validation_data: DataLoader) -> {str, float}:
    model.eval()
    with torch.no_grad():
...
def train_model(model: nn.Module, optimizer: optim.Optimizer, scheduler: lr_scheduler,
    ...
    criterion = LargeMarginSoftmaxLoss(m=LargeMarginSoftmaxLoss_M)
    ...
    for ep in range(N_EPOCHS):
        ...
        training_scores = fit(model=model, optimizer=optimizer, training_data=training_data, criterion=criterion)
        validation_scores = evaluate(model=model, validation_data=validation_data, criterion=criterion)
        scheduler.step()
...
def save_scores_and_hyperparameter(scores: {str, int}, path: str):
    ...
    hyperparameter = {"number_of_epochs": N_EPOCHS,
                      ...
                      "loss_parameter_m": LargeMarginSoftmaxLoss_M}
...
def main():
    ...
    print("Preparing resnet18")
    classifier = prepare_model()
    optimizer = optim.Adam(classifier.parameters(), lr=LEARN_RATE, weight_decay=WEIGHT_DECAY)
...
def main_with_args(batch_size: int, lr: float, lr_step: int, lr_gamma: float,
    ...
    classifier = prepare_model()
    optimizer = optim.Adam(classifier.parameters(), lr=lr, weight_decay=w_decay)
    ...
    params = {"batch_size": batch_size, "learning_rat": lr, "learning_rat_step": lr_step,
              "learning_rat_gamma": lr_gamma, "weight_decay": w_decay}
