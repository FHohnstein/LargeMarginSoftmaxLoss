...
#from imbalanced_cifar10.new_loss.large_margin_softmax import LargeMarginSoftmaxLoss, AdaptedVGG16
from large_margin_softmax import LargeMarginSoftmaxLoss, AdaptedVGG16
...
def fit(model: torch.nn.Module, criterion: Callable, optimizer: torch.optim.Optimizer, training_data: DataLoader) -> {str, float}:
    model.train()
...
def train_model(model: nn.Module, optimizer: optim.Optimizer, scheduler: lr_scheduler,
    ...
    for ep in range(N_EPOCHS):
        ...
        training_scores = fit(model=model, criterion=criterion, optimizer=optimizer, training_data=training_data)
        validation_scores = evaluate(model=model, criterion=criterion, validation_data=validation_data)
        scheduler.step()
